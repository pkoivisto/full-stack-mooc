...
const resolvers = {
  Query: {
    ...
    allAuthors: () => {
      ...
      return authors.map(author => { return {...author, bookCount: booksByAuthor[author.name] } })
    },
  },
  Author: {
    bookCount: ({ bookCount }) => {
  ...
  Mutation: {
    ...
    editAuthor: (root, { name, setBornTo }) => {
      const idx = authors.findIndex(author => name === author.name)
      if (idx === -1) {
        return null
      }
      const author = { ...authors[idx], born: setBornTo }
      authors[idx] = author
      return author
    }
  }
